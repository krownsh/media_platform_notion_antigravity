 ...
 * @param {string} options.userId - authenticated owner whose records may be processed
 */
export async function batchClassify({ ruleOnly = false, limit = 100, userId } = {}) {
    if (!userId) throw new Error('Authenticated user ID is required for batch classification');
