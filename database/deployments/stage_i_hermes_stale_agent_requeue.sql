-- Stage I follow-up: requeue a dispatch when the single Agent lease expires.
-- This prevents a delivered webhook from becoming permanently invisible when
-- Hermes crashes before it can run agent:webhook or finish the Agent lease.
begin;
...
begin
    ...
    where dispatch.attempt_count < dispatch.max_attempts
      ...
      and dispatch.status in ('pending', 'failed')
