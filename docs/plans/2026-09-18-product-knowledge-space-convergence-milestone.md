...
| M1 | Extensible product-path model | Product stages, gate definitions, transitions, and node links are data-driven; a post-release feedback edge is representable | Additive schema/migration; service/API contracts; Docker tests; explicit coverage-gap behavior | Isolated Docker plus later live additive-schema readback | Pending | Design and test the stage/transition model |
| M2 | Knowledge-space frontend entry | An authenticated user can discover available spaces without knowing an ID, then enter a path reader from desktop and mobile navigation | Space-list API; `/knowledge-spaces` route; desktop/mobile navigation; route/UI/build Docker tests | Docker/build; later deployed authenticated browser verification | Pending | Implement entry API, index page, and navigation |
| M3 | Path reader as one connected journey | Reader renders ordered stages, gate cards, source-backed nodes, feedback edge, and honest evidence gaps; it does not render a disconnected node list | Reader API extension; stage UI; cited-node links; gap state; Docker tests | Docker/build; later deployed authenticated browser verification | Pending | Map current 6 nodes and evidence; identify unsupported stages |
| M4 | Taxonomy reconciliation preview | Every proposed rename, post reassignment, and zero-post cleanup candidate is frozen and reviewable before any live mutation | Read-only manifest with stable IDs, original/target names, per-post route/confidence/reason, candidate checksums, exclusion set, and candidate post counts | Read-only live DB export; no rename/reassignment/removal | Pending | Generate and present manifest to Owner |
