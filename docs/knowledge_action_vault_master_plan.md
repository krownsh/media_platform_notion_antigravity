...
日期：2026-07-24
狀態：產品目標與核心流程已重新確認；Stage B SQL 已由使用者回報執行，待部署端 smoke test 驗證
...
## 0. Stage A 實作狀態（2026-07-24）

- `/api/process` 已要求 Supabase JWT，或具 server-side user mapping 的 capture API key；不再採信 body `userId`。
...
1. 依 [Stage B 部署說明](stage_b_source_finalization_deployment.md) 檢查 `source_domains` 為 `text[]`，再部署 backend 的 `server/.env` 與程式。
2. 依 [n8n Capture 設定](n8n_capture_setup.md) 發送一筆公開 URL，保留 `x-correlation-id`，並完成 RPC／outbox idempotency smoke test。
3. 從驗證完成的遠端 schema 回填正式 Supabase migration history。
4. 決定第一批允許 Project Auditor 掃描的專案根目錄，並以本 repo 進行第一輪 read-only dry-run。
5. 確認 Codex Local Agent Runner 的啟動方式：先互動式 session，或加入排程 CLI。

本輪已確認但尚未進入 Stage B–E 實作：
...
- 正式 Supabase schema 是否完全等同 repo 內的 [`database/schema/schema.sql`](../database/schema/schema.sql)。
...
Project Auditor 不能只讀使用者手寫的 [`docs/project/tasks.md`](project/tasks.md)。它必須定時掃描整個允許專案，主動找出：
...
Auditor 不得自行修改受保護的 [`docs/project/tasks.md`](project/tasks.md)，而是寫入獨立 Project Needs Ledger：
...
1. **程式已修復，待部署驗證**：`/api/process` 現在要求 JWT 或已映射的 API key，並忽略 body `userId`。
2. **SQL 已準備，待套用**：`original_url` 需由全域 unique 換成 `(user_id, original_url)`；orchestrator/manual lookup 已改為 tenant-aware。
3. **Stage B 本機已準備，待 staging 套用**：`finalize_collection_capture(...)` 將 post、analysis、media、comments 與 `source.ingested.v1` outbox 包在單一 DB transaction；仍需完成實際 DB smoke／idempotency 驗證與正式 migration history。
4. **已移除**：Gemini fallback、SDK、環境依賴與模型選單。
5. **程式已加嚴，仍待 Structured Output**：AI response 必須是完整 JSON object；後續需為各 job type 定義 JSON schema 與 source finalization status。

部署前仍需檢查既有資料是否已造成缺漏、跨 tenant 污染或 mock analysis，並確認重跑不會覆寫人工資料。
...
- 不寫入正式 Mac Vault。
- 不調整 [`docs/project/tasks.md`](project/tasks.md)。
- 不自動 Commit 或 Push。
