...
它建立 `collection_topic_scopes`。現有的 `collection_collections` 資料夾與既有收藏都不會被修改。

## 三種模式

- `collect`：只保存來源。不能研究、專案比對、產生或執行 POC。
- `research`：保留作研究題材；不會比對專案或產生 POC。
- `poc_proposal`：可針對 `project_targets` 中明確指定的專案，做本機唯讀盤點與 POC 提案。
...
2. 對每個資料夾選擇模式，並填入主題目標。
3. 只有在選擇 `POC 提案` 時，填入要允許比對的專案代號，例如 `media_platform_notion_antigravity`。
4. 按「儲存」。
