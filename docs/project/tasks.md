...
- [x] Gemini 移除後，將 `aiService.getAvailableModels()`、`aiService.generateImageFromPrompt()` 與 image workflow routes 明確退役（HTTP 410），避免留下無效路由。
- [x] 移除 `ImageWorkflowPage` 的 `user-id-placeholder`；整個 workflow 因未設定替代影像供應商而停用。
