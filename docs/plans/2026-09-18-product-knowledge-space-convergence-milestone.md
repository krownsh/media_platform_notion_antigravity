...
| M1 | Extensible product-path model | Product stages, gate definitions, transitions, and node links are data-driven; a post-release feedback edge is representable | Docker: 3 passed, 0 failed; **live** `stage_w4_knowledge_space_path` + RPC-revoke migration applied; readback: 7 stages / 6 mappings / 7 transitions / 1 feedback; anon/authenticated execute access to both internal guard functions = false; SSD backup `/Volumes/DevSSD/hermes/backups/knowledge-space-path-migration/2026-09-18T060000Z/pre-migration-backup.json` SHA-256 `60bbf6afbf9f81b0f7ef3533f61d2fe8b5d1b9af26f2a924720f461171155821` | Isolated Docker contract + live schema/readback | **Partial** | Deployed API and authenticated browser verification remain pending |
...
| M3 | Path reader as one connected journey | Reader renders ordered stages, gate cards, source-backed nodes, feedback edge, and honest evidence gaps; it does not render a disconnected node list | `KnowledgeSpaceMap`; stage journey contract; **live seed readback:** 7 stages / 6 exact owner-space-safe node mappings / 7 transitions with 1 feedback edge; Docker contracts: 5 passed, 0 failed; clean-container Vite build: passed (2.11s) | Isolated Docker contract + live DB readback | **Partial** | Deployed API and authenticated browser verification are pending |
| M4 | Taxonomy reconciliation preview | Every proposed rename, post reassignment, and zero-post cleanup candidate is frozen and reviewable before any live mutation | `docs/previews/2026-09-18-live-taxonomy-manifest.json`; 2026-09-18 live read: 430 posts / 333 assigned / 97 Inbox / 0 dangling; 0 renames; 0 reassignment; 33 exact zero-post Hermes candidates; live fingerprint `a3175a2e59ee172a75344865c7be9c41`; artifact SHA-256 `249534906f932bf8abe2699edd7dea87a895d15f49119d6c55d96531d494cc93`; Owner approved scope; recheck matched | Read-only export + Owner-approved scope | **Complete** | Writeback is independently blocked: no reversible cleanup column exists and physical deletion is prohibited |
| M5 | Owner-approved live taxonomy writeback | Only manifest-approved collection changes apply; unrelated records remain untouched; before/after cardinality and ownership checks pass | Owner approved scope; recheck: 33 candidates / `a3175a2e59ee172a75344865c7be9c41` / 0 nonzero; SSD backup at `/Volumes/DevSSD/hermes/backups/live-taxonomy-cleanup/2026-09-18T054515Z/`; `backup-metadata.json` SHA-256 `ca32a21ee1892a634133f3f446d46f1412018c520e0e1989169c7c16fb8b17ae` | Authorized scope; no live mutation performed | **Blocked by safety boundary** | Schema has no soft-archive field; only physical delete exists and is prohibited by the global no-delete boundary |
| M6 | Zero-post Hermes-folder cleanup | Only confirmed zero-post, Hermes-created duplicate candidates are removed from active live collection state; every candidate is backed up and post count rechecked inside the write boundary | Same M5 backup and recheck; `collectionVisibility.js` already hides descriptions containing `Hermes 自動建立` | No live mutation performed | **Blocked by safety boundary** | No reversible cleanup representation exists; do not substitute a fake archive value or physical delete |
