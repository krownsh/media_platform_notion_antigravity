# Documentation index

## Active operation

- [Knowledge Action Vault master plan](knowledge_action_vault_master_plan.md)
- [Crawler and capture pipeline](process_crawler_pipeline.md)
- [n8n capture setup](n8n_capture_setup.md)
- [Supabase integration](supabase_integration.md)
- [Stage B source-finalization deployment](stage_b_source_finalization_deployment.md)
- [Stage C Content Studio deployment](stage_c_content_studio_deployment.md)
- [POC generation and sandbox workflow](poc_sandbox_workflow.md)
- [Hermes pull-based outbox workflow](hermes_outbox_workflow.md)

## Architecture

- [API flow diagram](architecture/api_flow_diagram.md)
- [System agent responsibilities](../agents.md)

## Frontend

- [Design system](frontend/design_system.md)
- [Art direction](frontend/art_direction.md)
- [UI theme implementation update](ui_theme_update.md)

## Project reference

- [Project memory](project/memory.md)
- [Questions and troubleshooting](project/questions.md)
- [Development task list](project/tasks.md)

## Historical records

- [Full JSON refactor report](history/full_json_refactor_report.md)
- [Task logs](../Task_Logs/)
- [Archived server artifacts](../server/archive/README.md)

Database schema and deployment SQL live in [database/](../database/README.md),
not in this documentation tree.
