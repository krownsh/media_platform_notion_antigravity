...
begin
    ...
    -- Link an existing tenant-owned topic only. A title or slug is a
    -- suggestion; unattended processing must never materialize a Topic.
    ...
    v_topic_title := left(nullif(btrim(coalesce(v_topic_input ->> 'suggested_title', v_topic_input ->> 'title')), ''), 240);
    v_topic_slug := left(nullif(btrim(v_topic_input ->> 'slug'), ''), 120);

    if (v_topic_input ->> 'topic_id') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' then
        v_topic_id := (v_topic_input ->> 'topic_id')::uuid;
        select * into v_topic
        from public.collection_topics
        where id = v_topic_id and user_id = v_post.user_id
        limit 1;
    elsif v_topic_slug is not null and v_topic_confidence >= 0.85 then
        select * into v_topic
        from public.collection_topics
        where user_id = v_post.user_id and slug = v_topic_slug and status = 'active'
        limit 1;
    end if;
    ...
    if v_topic.id is not null then
        ...
        insert into public.collection_topic_source_matches (
            ...
            left(coalesce(nullif(v_relation_input ->> 'rationale', ''), v_topic_input ->> 'rationale', 'Codex link-only topic assignment'), 4000),
    ...
    -- Reuse an explicit tenant-owned Collection or an exact duplicate's
    -- established Collection. A free-text folder suggestion remains unfiled.
    ...
    if (v_folder_input ->> 'collection_id') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' then
        v_collection_id := (v_folder_input ->> 'collection_id')::uuid;
        select * into v_collection
        from public.collection_collections
        where id = v_collection_id and user_id = v_post.user_id;
    elsif v_duplicate.collection_id is not null then
        select * into v_collection
        from public.collection_collections
        where id = v_duplicate.collection_id and user_id = v_post.user_id;
    end if;
    if v_collection.id is not null
        and v_folder_confidence >= 0.85
        and v_collection.id = any(array[
    ...
    v_context := coalesce(v_workflow.context, '{}'::jsonb) || jsonb_build_object(
        ...
        'classification_suggestions', jsonb_build_object(
            'folder', case when v_collection.id is null and v_domain is not null then jsonb_build_object('suggested_name', v_domain, 'confidence', coalesce(v_folder_input -> 'confidence', 'null'::jsonb)) else null end,
            'topic', case when v_topic.id is null and v_topic_title is not null then jsonb_build_object('suggested_title', v_topic_title, 'confidence', coalesce(v_topic_input -> 'confidence', 'null'::jsonb)) else null end
        ),
        'folder_persistence', jsonb_build_object('assigned', v_collection.id is not null, 'collection_id', v_collection.id, 'name', v_collection.name, 'inherited_from_duplicate', v_duplicate.id, 'reason', case when v_collection.id is null then 'no_existing_collection' else null end),
...
comment on function public.codex_stage_collection_preprocess(uuid, jsonb, text) is
    'Stage O DB-only preprocess: preserves Stage M lifecycle while linking tenant-owned containers only; free-text suggestions stay in workflow context.';
