...
狀態：已完成並合併至本機 `main`（`af7013d`）；未 push、未部署，也未修改資料庫或實際 Vault。本文件保留原始回復範圍與驗收紀錄。
...
- 已回復的本機程式尚未 push／部署；不得把已撤回的 `wiki/sources/<post-id>.md` 版本部署到 Hermes／Mac。

## 執行任務

- [x] 建立隔離 worktree，確認沒有與其他 session 的修改重疊。
- [x] 以內容分類改動前的 writer contract 為基準，恢復 `wiki/threads/<platform>/<日期>-<標題>--<post-id前8碼>.md`。
- [x] 移除 `wiki/collections/<collection-id>.md`、`wiki/inbox.md` 自動索引寫入，以及 Collection 改名／移動時的 Vault index 更新。
- [x] 恢復原本內容草稿路徑規則；不搬現有草稿。
- [x] 停用並移除這次新增的 Vault 舊路徑 migration／DB-only manifest 執行入口，保留既有 DB 與檔案原狀。
- [x] 保留 `replication_plan` 的前端 badge、詳細頁資訊，以及 workflow JSON 的 goal／MVP／acceptance criteria。
- [x] 更新 Vault writer、workflow、skill、migration 文件，明確標示「Collection 分類不驅動 Vault 路徑」。
- [x] 更新測試，鎖住 platform 路徑、無 Collection index、無自動搬檔，以及復刻方案前端仍存在。
- [x] 執行相關 Node tests、`npm run lint`、`npm run build`；不跑 Docker。
- [x] 完成隔離驗證後，依 Owner 指示合併至本機 `main`；未 push、不部署、不操作真實 Vault。
