...
## 2026-09-04：主題工作區的用途與目前能力

### 使用者提問

「我還是不理解主題工作區的功能及價值及用法。」

### 說明

- 主題工作區不是 AI 分類標籤，也不是貼文資料夾；它應是使用者明確定義的長期工作目標，例如「建立 AI 工作流程自動化」。
- 一個主題未來應串連相關來源、研究問題、POC 候選與內容角度，讓後續 Agent 工作有明確脈絡，而不是只依單篇貼文猜測。
- 目前產品只完成「手動建立與列出主題」以及不寫入資料庫的來源匹配預覽。使用者可從側欄進入「主題」，填入名稱、目的、關鍵字與預期成果後建立。
- 現階段尚未完成接受來源匹配、編輯／封存主題，或讓工作流實際採用主題內容。因此它目前是資料模型與介面的 MVP，尚未具備足以日常使用的工作價值；後續應先完成「接受貼文匹配」與「在研究／POC／內容草稿中使用已接受的主題」才適合推廣使用。

### 2026-09-04 更正：實際運行中的自動主題建立

- 前述「目前只完成手動建立與列出」不符合實際資料庫。唯讀盤點確認：共有 60 個主題、單一 owner，全部由 `agent_auto` 建立。
- Stage K／M 後續加入的 Hermes／Codex preprocess 會為高信心結果建立 active topic，並把分數達門檻的來源匹配直接設為 `accepted`；現有匹配為 35 個 `accepted`、25 個 `suggested`。
- 這正是主題過度零碎的根因。後續改造須改為 agent 只提出候選，使用者才可接受匹配或建立主題。

## 2026-09-04：Collection／Topic／container／slug 的差別

### 使用者提問

「我有點搞混了 Collection / Topic / 容器 / slug 這些的差別了」

### 說明

- **Collection** 是使用者手動整理貼文的資料夾／看板欄位，例如「AI 工具」、「投資」。它回答「這篇收藏要放在哪裡」，目前已可日常使用。
- **Topic** 是一個使用者擁有的長期工作脈絡，例如「建立 AI 工作流程自動化」。它可累積相關來源、研究問題、POC 候選與內容角度；不是標籤，也不是貼文資料夾。現階段只完成建立、列出與不寫入的來源匹配預覽，尚未真正驅動後續工作流。
- **container／容器** 不是本產品一個固定的使用者概念：前端程式中通常指 UI 外層元素；爬蟲中指抓取到的 DOM 區塊；Docker 文件中指隔離執行環境；社群發布 API 中則可能指平台建立貼文前的媒體容器。不要把它當 Collection 或 Topic 的同義詞。
- **slug** 是給系統使用、穩定且可讀的短代號，例如 Topic「建立 AI 工作流程自動化」可用 `ai-workflow-automation`。它用於 URL、查找與同一使用者範圍內的唯一識別；不是顯示名稱，也不是標籤本身。
